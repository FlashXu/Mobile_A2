...
class SignUp extends Component {
    constructor(props) {
        ...
        if(this.data.hasOwnProperty('email')){
          ...
          email:this.data.email, gender:'male', firstname: this.data.first_name, lastname: this.data.last_name,
          sendingRequest:false, pwd:'',pwd2:'' };
        }else{
    ...
    if (!this.state.firstname){ 
      ...
      this.bodyOperation(url, data, 'POST').then((results)=>{
        ...
        if (status == 406){
          ...
          if(this.data.hasOwnProperty('photoUrl')){
            var data = JSON.stringify({
              "_id": results.gen_id,
              "email": user_name,
              "gender": gender,
          ...
          post_func(url, data, 'POST').then(() => {
            storeinfo('@accountID', results.gen_id);
            navi('MainMenuPage');
          });
        }
      }, user_name, first_name, last_name, gender, post_func, storeinfo, navi);
    }
  }
